...
- Ruff、61 項 pytest、14 項 Vitest 與正式前端 build 通過。
- 原始碼及打包 EXE 的 WebView2 自測通過，建立 `9月/底座` 與 `9月/探頭` 後，
  案件照片成功儲存並由清單讀回。
- 使用已簽章的 0.4.0 更新包完成兩輪隔離升級；一般更新器及凍結的舊版更新器皆通過。
- SQLite 原資料、更新前備份、使用者照片及舊程式均保留，新版啟動健康檢查成功。
- 完整安裝 ZIP 共 316 個項目，更新 ZIP 共 314 個項目；未包含資料庫、照片、金鑰或
  測試更新器。
- Windows 11 x64 已實測；Windows 10、斷電過程與各家防毒鎖檔情境仍待實機驗證。
